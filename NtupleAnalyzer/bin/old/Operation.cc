...
namespace Operation 
{
  ...
  int JetIndex(int ind, EventData & ev){
	...
	leadJet.SetPtEtaPhiE(ev.PFAK5JetPtCor(ixjet1), ev.PFAK5JetEta(ixjet1) , ev.PFAK5JetPhi(ixjet1) , ev.PFAK5JetE(ixjet1));
	for (int i=0; i<ev.NPFAK5Jets(); i++){
	  if(i<=ixjet1) continue;
	  radJet.SetPtEtaPhiE(ev.PFAK5JetPtCor(i), ev.PFAK5JetEta(i), ev.PFAK5JetPhi(i), ev.PFAK5JetE(i));
	  if(ev.PFAK5JetPtCor(i)>30.0 && abs(ev.PFAK5JetEta(i))<2.5 && radJet.DeltaR(leadJet)<1.1){
	    leadJet+=radJet;
	    continue;
	  }
	  if(ev.PFAK5JetPtCor(i)>ev.SecJetCut() && abs(ev.PFAK5JetEta(i))<4.5 && LepInJet2(i,ev)==false){
	    if(njets==ind){
	      send=i;
	    }
	    njets++ ;
  ...
  int JetNumber(EventData & ev){
	...
	if(ixjet1<99){
	  ...
	  for (int i=0; i<ev.NPFAK5Jets(); i++){
	    ...
	    if(checkWideJet==true && ev.PFAK5JetPtCor(i)>ev.SecJetCut() && abs(ev.PFAK5JetEta(i))<4.5 && LepInJet2(i,ev)==false) 
	      njets--;
	  }
	}
      }
	...
	if( ixjet1<99 ){
	  ...
	  for (int i=1; i<ev.NPFAK5Jets(); i++){
	    ...
	    //all jet conditions
	    if(ev.PFAK5JetPtCor(i)<=ev.SecJetCut()) continue;
	    if(abs(ev.PFAK5JetEta(i)) >= 4.5) continue;
	    if(LepInJet2(i,ev)) continue;
	    //cout<<"jet > 60GeV and not in leading jet: "<<  ev.PFAK5JetPtCor(i) << " "<<ev.PFAK5JetEta(i)<<" "<<ev.PFAK5JetPhi(i)<<endl;
	    njets++;       
	  }
	}
      }
  ...
  int WideJetNumber(EventData & ev){
	...
	for (int i=0; i<ev.NPFAK5Jets(); i++){
	  ...
	  if(i==ixjet1) continue;
	  radJet.SetPtEtaPhiE(ev.PFAK5JetPtCor(i), ev.PFAK5JetEta(i), ev.PFAK5JetPhi(i), ev.PFAK5JetE(i));
	  if(ev.PFAK5JetPtCor(i)>30.0 && abs(ev.PFAK5JetEta(i))<2.5 && radJet.DeltaR(leadJet)<1.1 && LepInJet2(i,ev)==false){
	    leadJet+=radJet;
	    if(ev.PFAK5JetPtCor(i)>ev.SecJetCut())
	      checkWideJet=true;
	  }
  ...
  void Manager::Run(EventData & ev) {
	...
	++i;
      ...
      //<<  deltaPhi( ev.PFAK5JetPhi(ixjet1) ,  ev.PFAK5JetPhi(ixjet2) )  << "  jetPt: "  <<  ev.PFAK5JetPtCor(ixjet1) 
      //<<  "   "    << ev.PFAK5JetPtCor(ixjet2)   <<endl; 
  ...
  bool CutNoiseClean::Process(EventData & ev){
	...
	if(njets==1)
	  ...
	  if(ev.PFAK5JetNeuHadEngFrac(ixjet2)< 0.7)
	    send=true;
	}
      }
    }
  ...
  bool CutJet2::Process(EventData & ev){
	...
	int ixjet1 = JetIndex(0, ev);
	int ixjet2 = JetIndex(1, ev);
	if( ixjet1<99 && ixjet2<99 && ev.PFAK5JetPtCor(ixjet2)>mJetPt && fabs(ev.PFAK5JetEta(ixjet2))<mJetEta ){
	  send=true;
	}
      }
      else
  ...
  bool CutDeltaPhi2::Process(EventData & ev){
    ...
    if( ev.JetType()=="pf"){
      ...
      int ixjet2= JetIndex(1, ev);
      if(JetNumber(ev)==1) accept =true;
      if(ixjet2<99 &&  JetNumber(ev)>1 && abs( deltaPhi( ev.PFAK5JetPhi(ixjet2), 
  ...
  bool CutDeltaPhi3::Process(EventData & ev){
    ...
    if( ev.JetType()=="calo"){
      ...
      if(ixjet2<99 && ixjet1<99 && abs(deltaPhi(leadJet.Phi(), ev.PFAK5JetPhi(ixjet2)))<mCut1)
	accept=true;
      else 
	accept=false; 
